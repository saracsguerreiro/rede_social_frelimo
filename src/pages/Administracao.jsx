...
const Avatar = ({ initials, size = 36, bg = 'var(--red-600)' }) => (
  <div style={{ width: size, height: size, borderRadius: '50%', background: bg, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * 0.33, fontWeight: 700, flexShrink: 0 }}>{initials}</div>
);

const QuotasChip = ({ estado }) => (
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: estado === 'em_dia' ? 'var(--green-50)' : 'var(--red-50)', color: estado === 'em_dia' ? 'var(--green-800)' : 'var(--red-800)' }}>
    {estado === 'em_dia' ? <><CheckCircle size={11} /> Em dia</> : <><AlertTriangle size={11} /> Em atraso</>}
  </span>
);
...
function ModalMembro({ membro, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 440, boxShadow: '0 24px 64px rgba(0,0,0,0.25)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
        <div style={{ background: 'linear-gradient(135deg, var(--red-600), var(--red-800))', padding: '24px 24px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: '50%', width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff' }}><X size={15} /></button>
        </div>
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', marginBottom: 16 }}>
        ...
        <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Pesquisar por nome, cartão ou órgão..."
          style={{ width: '100%', padding: '10px 12px 10px 36px', border: '1.5px solid var(--gray-200)', borderRadius: 8, background: 'var(--gray-100)', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
          onFocus={e => e.target.style.borderColor = 'var(--red-600)'} onBlur={e => e.target.style.borderColor = 'var(--gray-200)'} />
      </div>
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {filtrados.length === 0 && <div style={{ padding: 24, textAlign: 'center', color: 'var(--gray-400)', fontSize: 13 }}>Nenhum membro encontrado.</div>}
        {filtrados.map((m, i) => (
          <div key={m.id} onClick={() => setAberto(m)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', cursor: 'pointer', borderBottom: i < filtrados.length - 1 ? '1px solid var(--gray-200)' : 'none' }}
      ...
      <ModalMembro membro={aberto} onClose={() => setAberto(null)} />
    </div>
  );
}
...
function SecaoQuotas({ mobile }) {
  ...
  const enviarGeral = () => { setNotifGeral(true); setTimeout(() => setNotifGeral(false), 3000); };
  const enviarIndiv = id => { setNotifId(id); setTimeout(() => setNotifId(null), 2500); };

  const Lista = ({ membros, tipo }) => (
    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
      {membros.length === 0 && <div style={{ padding: 20, textAlign: 'center', fontSize: 13, color: 'var(--gray-400)' }}>Nenhum.</div>}
      {membros.map((m, i) => (
        <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', borderBottom: i < membros.length - 1 ? '1px solid var(--gray-200)' : 'none' }}>
          <Avatar initials={m.avatar} size={34} bg={tipo === 'atraso' ? 'var(--red-600)' : 'var(--green-600)'} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{m.nome}</div>
            <div style={{ fontSize: 11, color: 'var(--gray-400)' }}>
              {tipo === 'atraso' ? `Último pag.: ${new Date(m.ultimoPagamento).toLocaleDateString('pt-PT')}` : `Pago em ${new Date(m.ultimoPagamento).toLocaleDateString('pt-PT')}`}
            </div>
          </div>
          {tipo === 'atraso' && (
            <button onClick={() => enviarIndiv(m.id)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 6, border: '1.5px solid var(--red-600)', background: notifId === m.id ? 'var(--red-600)' : 'transparent', color: notifId === m.id ? '#fff' : 'var(--red-600)', fontSize: 11, fontWeight: 700, cursor: 'pointer', flexShrink: 0, transition: 'all 0.2s' }}>
              <Bell size={11} /> {notifId === m.id ? 'Enviado!' : 'Notificar'}
            </button>
          )}
        </div>
      ))}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap: 12 }}>
        <div className="card" style={{ padding: '16px 20px', borderTop: '4px solid var(--green-600)', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div><div style={{ fontSize: 24, fontWeight: 800 }}>{emDia.length}</div><div style={{ fontSize: 12, color: 'var(--gray-400)' }}>Quotas em dia</div></div>
        </div>
        <div className="card" style={{ padding: '16px 20px', borderTop: '4px solid var(--red-600)', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div><div style={{ fontSize: 24, fontWeight: 800 }}>{emAtraso.length}</div><div style={{ fontSize: 12, color: 'var(--gray-400)' }}>Com atraso no pagamento</div></div>
        </div>
      </div>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h3 style={{ fontSize: 14, fontWeight: 800, color: 'var(--red-800)', display: 'flex', alignItems: 'center', gap: 6 }}>
            <AlertTriangle size={14} color="var(--red-600)" /> Em atraso — {emAtraso.length} militante(s)
          </h3>
          <button onClick={enviarGeral} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, background: notifGeral ? 'var(--green-600)' : 'var(--red-600)', color: '#fff', border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', transition: 'background 0.2s' }}>
            <Send size={12} /> {notifGeral ? 'Notificação enviada!' : 'Notificar todos'}
          </button>
        </div>
        <Lista membros={emAtraso} tipo="atraso" />
      </div>
      <div>
        <h3 style={{ fontSize: 14, fontWeight: 800, color: 'var(--green-800)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
          <CheckCircle size={14} color="var(--green-600)" /> Em dia — {emDia.length} militante(s)
        </h3>
        <Lista membros={emDia} tipo="emdia" />
      </div>
    </div>
  );
}
...
export default function Administracao({ setPage, mobile }) {
  ...
  return (
    <div>
      <div style={{ padding: '10px 16px', background: 'var(--yellow)', display: 'flex', gap: 8, alignItems: 'center' }}>
      ...
      {/* Submenu com o mesmo estilo do Marketplace */}
      <div style={{ background: 'var(--white)', borderBottom: '1px solid var(--gray-200)', display: 'flex', padding: '0 16px' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setSecao(t.id)} style={tabStyle(secao === t.id)}>{t.label}</button>
        ))}
      </div>
